...
import { createOrdertoPayment } from "../../Services/OrderService";
...
const Order = () => {
  ...
  const shippingAddress = storedShippingAddress
  ...
  const [address, setAddress] = useState(() => ({
    ...
    phone: shippingAddress?.phone || user?.phoneNumber || "",
    detail: shippingAddress?.detail || user?.address || "",
  }));

  const [newAddress, setNewAddress] = useState(address);

  //Fetch cart + BlindBoxDetail
  useEffect(() => {
  ...
  const handlePlaceOrder = async () => {
    if (!address.name || !address.phone || !address.detail) {
      alert("⚠️ Please update your shipping address before placing an order!");
      return;
    }
    const orderPayload = {
    ...
    try {
      const response = await createOrdertoPayment(orderPayload);
      if (response.success && response.data?.orderId) {
        const newOrder = { ...orderPayload, orderId: response.data.orderId };
  ...
  return (
    <div className="order-container container mt-4 p-3 rounded shadow bg-white">
